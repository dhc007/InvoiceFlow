    ...
    <section className="w-full">
      <div className="flex flex-wrap gap-3 p-4">
        {testimonials.map((testimonial) => (
          <Card
            key={testimonial.id}
            className="flex-1 min-w-60 rounded-lg overflow-hidden border-none"
          >
            <div
              className="w-full h-[301px] rounded-xl bg-cover bg-center"
              style={{ backgroundImage: `url(${testimonial.image})` }}
            />
            <CardContent className="flex flex-col gap-2 pt-4 px-0">
              <p className="font-medium text-[#1c160c] text-base leading-6">
                {testimonial.quote}
              </p>
              <p className="font-normal text-[#9e8747] text-sm leading-[21px]">
                {testimonial.author}
              </p>
            </CardContent>
          </Card>
